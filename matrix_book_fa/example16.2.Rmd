...
در این مثال، یک مدل روند خطی محلی تخمین می‌زنیم.  برای این منظور،‌  داده‌های فصلیِ درآمد هر سهم  جانسون‌جانسون را که در `datasets::JohnsonJohnson` وجود دارد، به‌کار می‌بریم. برای تخمین، از بستهٔ نرم‌افزاری `dlm` <span dir="ltr">[@R-dlm]</span> استفاده می‌کنیم. کد زیر فرایند مدل‌سازی و تخمین را نشان می‌دهد:
```{r}   
...
```{r fig_local_linear, fig.align='center', fig.cap="", fig.width=5, fig.height=3}
